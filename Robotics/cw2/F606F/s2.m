...
% after platform
front_x2 = front_x + idealsize_bp*idealno_bp - 0.05 - 0.1;
rear_x2 = front_x2 + min_sep;
[idealsize_bp2, idealno_bp2]=no_steps(front_x2,end_all,max_x);
% move coordinates 
movecoor1 = position(front_x,rear_x, plat_h, ...
    idealsize_bp, idealno_bp, L_leg,...
    front_x2, rear_x2, idealsize_bp2, idealno_bp2);
%% Animation
...
plot([plat_sx,plat_sx,plat_ex,plat_ex],...
        [0,plat_h,plat_h,0],...
         'k','LineWidth',3);
plot([-1.05, -1.05], [0, 0.4], 'k-', 'LineWidth', 3);
...
% Flatsurface motion
function movecoor = position(frontx,rearx, height, distance, num, L_leg, frontx2,rearx2,dist2, num2)
% ------------------------------------- %
...
% Flat after
movecoor4 = zeros(4,4*num2+3);
movecoor4(1,1) = frontx2;
movecoor4(2,1) = rearx2;
movecoor4(2,:) = rearx2;
dim4 = size(movecoor4);
for i = 2:dim4(2)
    if rem(i,2) == 0
        if rem(i,4) == 0
            % Rear %
            % arrange x
            movecoor4(2,i) = movecoor4(2,i-1)+0.5*dist2;
            movecoor4(2,i+1:i+3) = movecoor4(2,i-1)+dist2;
            % arrange y
            movecoor4(4,i) = height;
        else
            % Front %
            % arrange x
            movecoor4(1,i) = movecoor4(1,i-1)+0.5*dist2;
            movecoor4(1,i+1:i+3) = movecoor4(1,i-1)+dist2;
            % arrange y
            movecoor4(3,i) = height;
        end
    end
end
movecoor4 = movecoor4(:,1:dim4(2));
movecoor4
% Up Steps 
movecoor2 = zeros(4,4);
feet2 = [];
for s = 1:4
    ...
    if rem(s,2) == 0
        ...
        if rem(s,4) == 0
            ...
            movecoor2(3,s-3:s) = height; 
            movecoor2(4,s-1:s) = height;
        else 
            feet2(s-1) = 'f';
            feet2(s)   = 'f';
...
for s = 1:4
    if s == 1
        movecoor3(:,s) = movecoor2(1:4,4);
        movecoor3(1,s) = movecoor3(1,s)-0.05;
    else
        movecoor3(:,s) = movecoor3(:,s-1);
    end
    if rem(s,2) == 0
        ...
        if rem(s,4) == 0
            feet3(s-1) = 'r';
            feet3(s)   = 'r';
            movecoor3(2,s-1:s) = movecoor3(2,s-1:s)-0.05; 
            movecoor3(4,s) = movecoor3(4,s)-height;
        else 
            feet3(s-1) = 'f';
            feet3(s)   = 'f';
            movecoor3(3,s) = movecoor3(3,s)-height; % 0.1 stair size
...
feet4(1) = 'f';
for i = 2:dim4(2)
    diff = movecoor4(1:2,i)-movecoor4(1:2,i-1);
    if diff(1) ~= 0
        feet4(i) = 'f';
    elseif diff(2) ~= 0
        feet4(i) = 'r';
...
movecoor4 = [movecoor4;feet4];
movecoor = [movecoor,movecoor2,movecoor3,movecoor4];
% ------------------------------------- %
% Add all angles info
